class CouncilSession < ApplicationRecord
  ...
  def change_of_affiliations
    events.where(eventable_type: "ChangeOfAffiliation")
  end
  ...
  def update_seat_commenced_on
    seats.where(commenced_on: [commenced_on_before_last_save, nil])
      .update_all(commenced_on: commenced_on)
  end

  def update_seat_concluded_on
    seats.where(concluded_on: [concluded_on_before_last_save, nil])
      .update_all(concluded_on: concluded_on)
  end
end
